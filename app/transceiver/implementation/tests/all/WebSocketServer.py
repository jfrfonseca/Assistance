...
class WebSocketServer(object):
   ...
   def clearMessageBuffer(self):
      while self.instance == '':
         time.sleep(self.warmupDelay)
      self.websocketclass.clearMessageBuffer(self.instance)
          
   def getMessageBuffer(self):
      while self.instance == '':
         time.sleep(self.warmupDelay)
      return self.websocketclass.getMessageBuffer(self.instance)
   
   def sendMessage(self, message):
      while self.instance == '':
         time.sleep(self.warmupDelay)
      self.websocketclass.sendMessage(self.instance, str(message))
         ...
         for ready in rList:
            if ready == self.serversocket:
               try:
                  ...
                  self.instance = self.constructWebSocket(newsock, address)
                  self.connections[fileno] = self.instance
